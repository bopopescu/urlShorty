$(document).ready(function() { 
	...
	var oldLong = $('#ORIGINAL_URL').val();
	var oldShort = $('#NEW_URL').val();
 /**
	...
	$('#SUGGESTIONS').click(function(e) {
		...
		if (text == ""){ //show error message if the user hasn't entered a long URL yet
        	...
        	getURLHeadInfo(text).done(function (data) {
				...
				// 	$(".errorMessage").text("Sorry! We are not able to generate recommendations at this time.");
				// 	$(".errorMessage").show();
				// }
			});
		}
...
**/
	...
	$('#GO').click(function(e) {  
		...
		window.longUrlValue = $('#ORIGINAL_URL').val();
		window.shortUrlVal = $('#NEW_URL').val();
		console.log(">>>>>>>>>>>>>>>>>", "oldShort: ",oldShort, "newShort: ", shortUrlVal, " newLong: ", longUrlValue);
		if (longUrlValue === ""){//make sure the oldURL has been entered, otherwise show error message
			...
			$.ajax({
				...
				success: function(data,textStatus, jqXHR){	
					if (data.answer === "Success"){//if edit was successful, show the results on the page
						$('#generated').show();
						$('#NEW_URL_ENTRY').text(shortUrlVal);
					}
...
**/
	$('#changeURL').click(function(e) {
		...
		editPage = "edit?shortURL=" + window.shortUrlVal + "&longURL=" + window.longUrlValue;
...
function showTwitterError() {
	console.log("Generating Twitter Error in DOM");
	$(".errorMessage").text("Sorry! We don't have any suggestions for you right now.");
}
